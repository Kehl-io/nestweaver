...
| `NESTWEAVER_UI_ALLOWED_HOSTS` | unset | Comma-separated extra hostnames admitted by the local UI for a deliberate reverse proxy. Requests otherwise require a loopback Host (`localhost`, `127.0.0.1`, or `[::1]`); an Origin, when present, must match Host. Configure the proxy to forward the browser's Host unchanged. |
...
nestweaver context simple.js             # seed from all symbols in a file (repo-relative path)
...
#   MANIFEST ENTRY FILES REACH EVERY ROUTE (nw-512). Through 10.0.3 the
#   `<db>.manifests.json` entry files were loaded ONLY on the `--no-daemon`
#   bypass; `tool_dead_code` called a variant that hard-codes an empty
#   manifest map, so the default CLI, MCP direct and MCP-via-daemon all walked
#   with no manifest roots and reported live code dead. Both call sites now
#   share one loader. Entry files are matched per REPO (nw-497) — a flat path
#   list let `index.js` in repo A root a same-path file in repo B.
#   A manifest sidecar that cannot be READ is now disclosed rather than
#   silently dropped (nw-500): `coverage` reads "degraded" and
#   `manifest_load_error` names the cause, on every route, and the CLI prints
#   it before any count. An ABSENT sidecar is normal and stays silent; only a
#   corrupt one discloses. This matters in one direction: losing entry files
#   moves LIVE symbols onto the unreachable list, the same unrecoverable
#   direction that makes this command refuse outright on a stale resolver.
...
# GET  /api/v1/events           → SSE stream (pagerank:recomputed, full_refresh)
