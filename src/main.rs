...
use nestweaver_engine::{
    BrainContextResult, BrainWatcher, ContextResult, FeatureContextResult, LookupResult,
    build_brain_context, build_context, build_feature_context, compute_clusters,
    discover_cross_domain_links, embedding::generate_embedding, generate_guide, generate_repo_map,
    incremental_index, index_directory, index_markdown_directory, list_repos, list_services,
    load_clusters, load_manifest_cache, lookup_symbol, save_clusters, search_symbols,
    suggest_links,
};
...
enum Commands {
    ...
    ListRepos {
        ...
        json: bool,
        #[arg(long, help = "Path to the database file [default: ./nestweaver.lbug]")]
        db: Option<PathBuf>,
    },
    ...
    Index {
        ...
        #[arg(
            long,
            help = "Path to the output database file [default: ./nestweaver.lbug]"
        )]
        db: Option<PathBuf>,
...
fn open_store(db: Option<&Path>) -> anyhow::Result<GraphStore> {
    let path = db.unwrap_or_else(|| Path::new("./nestweaver.lbug"));
...
fn run(cli: Cli) -> anyhow::Result<i32> {
    match cli.command {
        ...
        Commands::Pull {
            ...
            let db_path = db
                .as_deref()
                .unwrap_or_else(|| Path::new("./nestweaver.lbug"));
        ...
        Commands::GenerateGuide { db, output } => {
            let db_path = db.unwrap_or_else(|| PathBuf::from("./nestweaver.lbug"));
            let store = open_store(Some(&db_path))?;
...
fn run_brain(command: BrainCommands) -> anyhow::Result<i32> {
    match command {
        ...
        BrainCommands::Refresh {
            ...
            // Cascade-delete every Note belonging to this vault (by UID).
            // Compute vault_uid the same way the indexer does so we hit
            // the right Vault. Canonicalize for FSEvents parity.
            let canonical = std::fs::canonicalize(&path).unwrap_or_else(|_| path.clone());
            let v_uid = nestweaver_schema::vault_uid(&instance_id, &canonical.to_string_lossy());

            // Open the store, find the notes, cascade-delete each.
            let store = open_store(Some(&db_path))?;
        ...
        BrainCommands::Context {
            ...
            let db_path = db.unwrap_or_else(|| PathBuf::from("./nestweaver.lbug"));
            let store = open_store(Some(&db_path))?;
            match build_brain_context(&store, &seeds) {
                Ok(result) => {
