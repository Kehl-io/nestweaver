...
- **Freshness:** results are only as current as the index. A stale or behind-source repo shows in `coverage.stale_repos` / `coverage.repos_not_indexed`. Separately, NestWeaver 9.0.0 bumped `RESOLVER_GENERATION` to 4, so a graph indexed by an earlier release has edges the current resolver would not have written — including C/C++ `MEMBER_OF` and C++ `IMPORTS` edges that did not exist at all. `stale_check` does not detect this; `hub_nodes` / `bridge_nodes` report it as `rankings_stale`. Re-index with `nestweaver index --repo <path> --force`.
