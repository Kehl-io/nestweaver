...
| A full `brain refresh` (and `brain add` through the daemon) no longer leaves every note without its code links, and a code re-index no longer leaves notes without their links into the changed files: the daemon's code-link reconciler rebuilds exactly the notes whose links no longer match their text, in the background. While links are owed, `brain status` lists a `note→code links` row under `Watcher reconciliation pending` (counted in `skipped_notes.reconciliation_pending`), with the last error if a pass failed; it clears when a pass lands | Nothing to change; expect the row briefly after a refresh or re-index |
| Note-to-code links (`REFERENCES_CODE`) now follow explicit code mentions and project scope: a name links when written as code (a backtick span, a code block, or an identifier-shaped `snake_case`/`camelCase` name or a `call(` in prose) and it resolves within the note's project to at most 5 defining files; plain English words, URLs, wikilinks and string literals never link, and `main` is stoplisted. On a real brain this is ~700× fewer links (39 M → ~56 K) at ~97 % precision, and `brain_context` ranks the symbols a note actually names. **First start after upgrading:** the daemon removes the old links in batches of 50,000 and relinks every note in the background; `brain status` shows a `note→code links` row (`link rules upgraded to v2`, with progress) until it finishes — expect minutes on a large brain; queries keep working meanwhile. An interrupted migration resumes on the next start | To keep a link for a plain name, write it as code (`` `Processor` ``, `review()`) or put the note in the project whose repo defines it |
