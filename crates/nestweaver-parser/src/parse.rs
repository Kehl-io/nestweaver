...
pub fn parse_source(path: &Path, source: &str) -> Result<ParsedFile, ParseError> {
    ...
    // nw-435: bash and python have no "main"-shaped convention that reliably
    // marks a script as its own entry point. A bash file's top-level
    // statements execute unconditionally whether the file is run directly or
    // `source`d, and a Python module's top level executes on both direct
    // execution and import -- neither `detect_bash` nor `detect_python` had a
    // rule for that at all. `detect_bash` recognised only a function
    // literally named `main`, so a script whose top level is bare statements
    // -- the DOMINANT real-world bash idiom -- has no entry point and every
    // function it defines is unreachable by construction. Proven by a
    // controlled pair: a script defining and calling a function at its own
    // top level (no wrapper, no `source`) reported 100% dead with
    // `entry_points: 0`; the identical script with the call moved inside a
    // `main()` function reported 0% dead. The difference was the wrapper,
    // not the code.
    //
    // The fix mints ONE synthetic symbol per file spanning the whole source
    // and marks it an entry point. Its span is deliberately the WHOLE file
    // rather than just the gaps between real definitions:
    // `find_enclosing_symbol` (nestweaver-resolver) walks candidate symbols
    // in REVERSE start-line order and returns the first whose span contains
    // the reference line, so a widely spanning symbol that starts at line 1
    // is only ever chosen for a reference that is NOT already inside some
    // later, more specific real function -- the same shape Rust's `Module`
    // symbols already rely on for nested `fn`s, not a new resolver behaviour.
    // A real top-level call (e.g. `helper "$@"` after `helper() { ... }`)
    // then resolves to THIS symbol as its source instead of
    // `Enclosing::ModuleScope`, which `resolve_single_reference` drops
    // outright (nw-349 cause 1) -- so the callee gets a real CALLS edge and
    // becomes reachable. A file with no top-level calls at all still gets the
    // symbol and it stays a leaf: it costs one always-reachable node, not a
    // false reachability claim, and it is what makes `entry_points` for these
    // two languages honest rather than zero (nw-435's other half; see
    // `dead_code.rs`'s per-language coverage check).
    //
    // KNOWN GAP, NOT fixed here: `name = call(...)` at module level (nw-435's
    // own measured instance, `lbug_version = _get_lbug_version()` in
    // `scripts/pip-package/setup.py`) still does not resolve to this symbol.
    // `python.scm` captures `lbug_version` itself as a one-line `Variable`,
    // and in `find_enclosing_symbol` (nestweaver-resolver) the EXACT-match
    // branch accepts any symbol whose span contains the reference line with
    // no kind check -- unlike the DEGENERATE fallback, which nw-349 already
    // restricted to code-bearing kinds via `can_contain_code` -- so the call
    // resolves to `lbug_version` instead of to this wider module symbol.
    // `lbug_version` is itself never called by anything, so the callee still
    // reads dead. The bare-statement and `if __name__` forms are unaffected
    // because neither leaves a same-line Variable/Constant symbol for the
    // resolver to prefer. Fixing the assignment form needs a resolver change
    // outside this fix's permitted scope (extend `can_contain_code`'s
    // restriction to the Exact branch, or a narrower carve-out for
    // module-level `name = call(...)`), and belongs with nw-349's
    // reference-capture class.
    if matches!(lang, Language::Bash | Language::Python) {
        let (name, signature) = if lang == Language::Bash {
            ("<script>", "<bash script top level>")
        } else {
            ("<module>", "<python module top level>")
        };
        let end_line = tree.root_node().end_position().row as u32 + 1;
        symbols.push(RawSymbol {
            name: name.to_string(),
            ...
            is_entry_point: true,
            entry_point_kind: Some(EntryPointKind::Main),
