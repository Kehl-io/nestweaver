...
pub fn parse_markdown(rel_path: &str, source: &str) -> Result<ParsedNote, MarkdownParseError> {
    ...
    // 2. Parse frontmatter — best-effort, recover from errors.
    ...
    // 3. Title resolution: frontmatter > first H1 > filename stem.
    ...
    // 4. note_kind: frontmatter `type:` > path heuristic > General.
    ...
    // 5. Word count: split body on whitespace, ignoring HTML / fenced code is good enough for v1.
    let word_count = u32::try_from(body.split_whitespace().count()).unwrap_or(u32::MAX);

    // 6. Extract headings and sections from the body.
    let headings = extract_headings(body);
    let sections = extract_sections(body, &headings);

    // 7. Extract wikilinks per section + tags (inline + frontmatter).
    ...
    // 8. Aliases from frontmatter `aliases:`.
    let aliases = extract_aliases(&frontmatter_json);

    // 9. Code block language tags.
