...
/// Matches the `<script ...>` opening tag (with optional attributes like `lang="ts"` or `setup`).
...
/// Matches `export default` declarations (component definition).
static RE_EXPORT_DEFAULT: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"export\s+default\s+").unwrap());

/// Matches `defineComponent(...)` calls.
...
/// Parse a Vue Single-File Component (`.vue`) using regex-based extraction.
///
/// Extracts the `<script>` block content and scans it for:
/// - Exported functions/constants → [`SymbolKind::Function`]
/// - `defineComponent` / `export default` → [`SymbolKind::Class`] (component)
...
pub fn parse_vue(path: &Path, source: &str) -> ParsedFile {
    ...
    // Extract the component name from filename (e.g., "MyComponent.vue" → "MyComponent")
        ...
        for (idx, line) in script_content.lines().enumerate() {
            ...
            if !trimmed.starts_with("import") {
                for cap in RE_CALL.captures_iter(trimmed) {
                    let name = cap[1].to_string();
                    if !CALL_EXCLUDE.contains(&name.as_str())
...
fn extract_script_blocks(source: &str) -> Vec<(String, u32)> {
    ...
    while i < lines.len() {
        if RE_SCRIPT_OPEN.is_match(lines[i]) {
            ...
            blocks.push((content, start as u32 + 1)); // +1 because line after <script> is content start
            // Skip the </script> line
            i += 1;
        } else {
            i += 1;
        }
