...
//! (`https://github.com/acme/api`) on the other. Keying repo identity on the
//! raw URL string makes those two forms look like two different repos, so
//! merged results never coalesce and the same symbol shows up twice.
//!
...
//! URL forms produce the same key.
//!
...
pub(crate) fn normalized_repo_key(repo_url: &str) -> String {
...
pub(crate) fn repo_name(repo_url: &str) -> Option<String> {
