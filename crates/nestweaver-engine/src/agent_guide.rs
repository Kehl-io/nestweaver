...
pub fn generate_skill_with_tools(
    ...
    out.push_str("**Exact pins survive the token-budget cut.** The first map entry is always admitted; additional `matched_query: \"exact\"` pins are kept even after the budget is spent. Other entries still drop. Exact names are looked up directly, so a path-deboosted test/mirror file is still pinned; remaining misses are true name collisions beyond the resolver's cap of 5 exact matches.\n\n");
